-- Restore foreign key constraints after successful migration (SAFE VERSION)
-- This version handles the auth.users constraint issue by modifying the profiles table

-- First, let's modify the profiles table to not have a foreign key to auth.users
-- since the migrated user IDs don't exist in the target database's auth.users table
-- We'll keep the profiles table as a standalone table for now

-- Remove the foreign key constraint from profiles to auth.users (if it exists)
ALTER TABLE public.profiles DROP CONSTRAINT IF EXISTS profiles_id_fkey;

-- Now we can safely restore all other foreign key constraints
